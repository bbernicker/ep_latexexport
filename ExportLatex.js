...
const getLaTeXFromAtext = (pad, atext) => {
  const apool = pad.apool();
  const textLines = atext.text.slice(0, -1).split('\n');
  const attribLines = Changeset.splitAttributionLines(atext.attribs, atext.text);
  const tags = ['textbf', 'textit', 'underline', 'sout'];
  ...
  const headingtags = ['section', 'subsection', 'subsubsection', 'paragraph', 'subparagraph', 'subparagraph', 'blockquote'];
  const headingprops = [
    ['heading', 'h1'],
    ['heading', 'h2'],
    ['heading', 'h3'],
  ...
  const getLineLaTeX = (text, attribs) => {
    ...
    if (heading) { //these three lines may be harmful or unnecessary. Unclear why they were in LaTeX code but not Markdown
      assem.append('}');
    }
    ...
    // this breaks LaTeX math mode: $\sum_i^j$ becomes $\sum\_i^j$
  ...
  for (let i = 0; i < textLines.length; i++) {
    ...
    const lineContent = getLineLaTeX(line.text, line.aline);
...
const getPadLaTeX = async (pad, revNum) => {
  const atext = revNum == null ? pad.atext : await pad.getInternalRevisionAText(revNum);
  return getLaTeXFromAtext(pad, atext);
};

exports.getPadLaTeXDocument =
    async (padId, revNum) => await getPadLaTeX(await padManager.getPad(padId), revNum);
